...
class PDFExporterImpressoras:
    ...
    def _col_widths(self, mostrar_sku):
        if mostrar_sku:
            ...
            col_preco  = 2.8 * cm
            col_qtd    = 1.8 * cm
            col_modelo = CONTENT_W - col_sku - col_cod - col_img - col_avista - col_preco - col_qtd
            return [col_sku, col_cod, col_img, col_modelo, col_avista, col_preco, col_qtd]
        else:
            col_cod    = 2.5 * cm
            col_img    = 1.8 * cm
            col_avista = 3.0 * cm
            col_preco  = 3.0 * cm
            col_modelo = CONTENT_W - col_cod - col_img - col_avista - col_preco
            return [col_cod, col_img, col_modelo, col_avista, col_preco]
    ...
    def _build_header_row(self, mostrar_sku):
        ...
        if mostrar_sku:
            return [hdr("SKU"), hdr("CÓDIGO"), hdr(""), hdr("MODELO", TA_LEFT), hdr("À VISTA"), hdr("10x S/JUROS"), hdr("QTD")]
        else:
            return [hdr("CÓDIGO"), hdr(""), hdr("MODELO", TA_LEFT), hdr("À VISTA"), hdr("10x S/JUROS")]

    # ── Tabela ────────────────────────────────────────────────────────

    def _make_table(self, rows, col_widths, mostrar_sku, is_continuation=False):
        img_col   = 1 if not mostrar_sku else 2
        # avista_col: col 3 (sem SKU) ou col 4 (com SKU); preco_col logo após
        avista_col = 3 if not mostrar_sku else 4
        preco_col  = avista_col + 1

        cmds = [
            ("VALIGN",        (0, 0),  (-1, -1),               "MIDDLE"),
            ("ALIGN",         (0, 0),  (0, -1),                "CENTER"),
            ("ALIGN",         (img_col,   0), (img_col,   -1), "CENTER"),
            ("ALIGN",         (avista_col,0), (avista_col,-1), "CENTER"),
    ...
    def _build_row(self, imp, mostrar_sku):
        if mostrar_sku:
            return [self._sku_cell(imp), self._cod_cell(imp),
                    ...
                    self._preco_avista_cell(imp), self._preco_cell(imp, mostrar_sku),
                    self._qtd_cell(imp)]
        else:
            return [self._cod_cell(imp), self._img_cell(imp),
                    self._modelo_cell(imp),
                    self._preco_avista_cell(imp), self._preco_cell(imp, mostrar_sku)]
    ...
    def _preco_avista_cell(self, imp):
        avista = imp.get("preco_avista", "").strip()
        if avista and not avista.upper().startswith("R$"):
            avista = f"R$ {avista}"
        return Paragraph(avista or "—", ParagraphStyle(
            "IAvista", fontName="Helvetica-Bold", fontSize=12,
            textColor=PRETO, alignment=TA_CENTER, leading=16))
