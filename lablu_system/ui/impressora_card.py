"""
Card visual para exibição de impressora em lista compacta.
"""
...
class _ImageHoverLabel(QLabel):
    ...
    def _show_popup(self):
        ...
        self._popup.setStyleSheet(
            "background: white; border: 2px solid #4F9900;"
...
class ImpressoraCard(QFrame):
    edit_requested    = pyqtSignal(int)
    delete_requested  = pyqtSignal(int)
    selection_toggled = pyqtSignal(int, bool)

    def __init__(self, impressora: dict, db, pixmap_cache: dict = None):
        super().__init__()
    ...
    def set_selection_mode(self, enabled: bool):
        ...
        if not enabled:
            self._is_selected = False
        self._update_visual(bool(self.impressora.get("em_estoque", 1)))
        self.setCursor(
    ...
    def mousePressEvent(self, event):
        if self._in_selection_mode and event.button() == Qt.MouseButton.LeftButton:
            ...
            self._update_visual(bool(self.impressora.get("em_estoque", 1)))
            self.selection_toggled.emit(self.impressora["id"], self._is_selected)
    ...
    def _build(self):
        ...
        # ── Estoque ─────────────────────────────────────────────────────
        em_estoque = bool(self.impressora.get("em_estoque", 1))
        ...
        # ── SKU ─────────────────────────────────────────────────────────
        sku_lbl = QLabel(self.impressora.get("sku", "").strip() or "—")
        ...
        # ── Código ──────────────────────────────────────────────────────
        cod_lbl = QLabel(self.impressora.get("marca", "").strip() or "—")
        ...
        # ── Imagem ──────────────────────────────────────────────────────
        img_path = self.impressora.get("imagem_path", "")
        ...
        # ── Modelo ──────────────────────────────────────────────────────
        modelo_lbl = QLabel(self.impressora.get("modelo", "").strip())
        modelo_lbl.setObjectName("cardName")
        modelo_lbl.setFixedWidth(COL_MODELO)
        ...
        # ── Quantidade ──────────────────────────────────────────────────
        qtd = self.impressora.get("quantidade", "").strip()
        ...
        edit_btn.clicked.connect(lambda: self.edit_requested.emit(self.impressora["id"]))

        del_btn = QPushButton("✕")
        del_btn.setObjectName("cardDeleteBtn")
        del_btn.setFixedWidth(32)
        del_btn.clicked.connect(lambda: self.delete_requested.emit(self.impressora["id"]))
    ...
    def _on_estoque_changed(self, state):
        ...
        self.db.set_estoque_impressora(self.impressora["id"], em_estoque)
        self.impressora["em_estoque"] = 1 if em_estoque else 0
        self._update_visual(em_estoque)
